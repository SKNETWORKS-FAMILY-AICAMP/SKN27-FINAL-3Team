...
def merit_classification_node(state: AppealJudgmentState) -> dict:
    """MG — notice_stage × 위반유형(law_code로 판별)별 참조 법조문을 컨텍스트로
    LLM 단일 호출 (DATA-003 §5, §9-2, §9-6).

    law_code는 인용 근거가 아니라 위반유형(주정차/비주정차) 판별용 라우팅 신호로만
    쓰인다 — MG는 이 고지서 개별 law_code를 인용하지 않고, 항상 사전에 검증된
    고정 조문(law_refs.py)만 LLM 컨텍스트로 주입한다.
    """
    reason = state.get("user_appeal_reason") or ""
    notice_stage = state.get("notice_stage")
    law_code = state.get("law_code")

    law_context = get_merit_context(notice_stage, law_code)
