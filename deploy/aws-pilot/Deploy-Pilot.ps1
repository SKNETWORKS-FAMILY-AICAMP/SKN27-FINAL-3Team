...
try {
    ...
    elseif ($StageForReleaseUpdate) {
        ...
        $commands += @(
            ...
            "rollback_previous_release() { status=`$?; trap - ERR; cd `$RELEASE_DIR; $productionComposeCommand down >/dev/null 2>&1 || true; if [ -n `"`$PREVIOUS_RELEASE`" ] && [ -d `"`$PREVIOUS_RELEASE`" ]; then cd `$PREVIOUS_RELEASE; $productionComposeCommand up -d --remove-orphans redis; wait_for_redis; $productionComposeCommand up -d --wait --wait-timeout 600 --remove-orphans `$WAITED_SERVICES; $productionComposeCommand up -d `$BACKGROUND_SERVICES; ln -sfn `$PREVIOUS_RELEASE /opt/skn27-pilot/current; fi; exit `$status; }",
