...
try {
    ...
    elseif ($StageForReleaseUpdate) {
        ...
        $commands += @(
            ...
            "PREVIOUS_RELEASE=`$(readlink -f /opt/skn27-pilot/current 2>/dev/null || true); [ -L /opt/skn27-pilot/current ] || PREVIOUS_RELEASE=''",
            ...
            "test -f `$RELEASE_EVIDENCE && test ! -L `$RELEASE_EVIDENCE",
            "LEGAL_DATASET_VERSION=`$(sed -n 's/^LEGAL_DATASET_VERSION=//p' `$RELEASE_DIR/.runtime.env); test -n `"`$LEGAL_DATASET_VERSION`"",
            "LEGAL_MAX_AGE_HOURS=`$(sed -n 's/^OPERATIONAL_LEGAL_MAX_AGE_HOURS=//p' `$RELEASE_DIR/.runtime.env); test -n `"`$LEGAL_MAX_AGE_HOURS`"",
            "cd `$RELEASE_DIR",
            "$stageComposeCommand --profile seed run --rm --no-deps -v `$RELEASE_DIR/operational-evidence:/run/release-evidence:ro rag-loader python -m etl.legal.validate_run_summary --summary /run/release-evidence/run_summary.json --max-age-hours `"`$LEGAL_MAX_AGE_HOURS`" --expected-dataset-version `"`$LEGAL_DATASET_VERSION`" --expected-release-version '$ReleaseTag'",
            ...
            "BACKGROUND_SERVICES='agent-worker file-scan-worker'",
            "MONITOR_SERVICE='ops-monitor'",
            ...
            "EVIDENCE_PROMOTED=0",
            "wait_for_redis() { for attempt in `$(seq 1 120); do $productionComposeCommand exec -T redis redis-cli ping | grep -qx PONG && return 0; if [ `$attempt -eq 120 ]; then echo 'Redis did not become ready during bootstrap.' >&2; return 1; fi; sleep 2; done; }",
            "promote_operational_evidence() { install -m 0444 `$RELEASE_EVIDENCE `$SHARED_EVIDENCE_TMP; $stageComposeCommand --profile seed run --rm --no-deps -v `$SHARED_EVIDENCE_DIR:/run/operational-evidence:ro rag-loader python -m etl.legal.validate_run_summary --summary /run/operational-evidence/.run_summary.json.tmp --max-age-hours `"`$LEGAL_MAX_AGE_HOURS`" --expected-dataset-version `"`$LEGAL_DATASET_VERSION`" --expected-release-version '$ReleaseTag'; mv -f `$SHARED_EVIDENCE_TMP `$SHARED_EVIDENCE_FILE; EVIDENCE_PROMOTED=1; }",
            "restore_operational_evidence() { if [ `$EVIDENCE_PROMOTED -ne 1 ]; then return 0; fi; if [ -n `"`$PREVIOUS_RELEASE`" ]; then test -f `$PREVIOUS_RELEASE/operational-evidence/run_summary.json; cp `$PREVIOUS_RELEASE/operational-evidence/run_summary.json `$SHARED_EVIDENCE_TMP; chmod 0444 `$SHARED_EVIDENCE_TMP; mv -f `$SHARED_EVIDENCE_TMP `$SHARED_EVIDENCE_FILE; else rm -f `$SHARED_EVIDENCE_FILE `$SHARED_EVIDENCE_TMP; fi; }",
            ...
            "rollback_previous_release() { status=`$?; trap - ERR; cd `$RELEASE_DIR; $productionComposeCommand down >/dev/null 2>&1 || true; restore_operational_evidence; if [ -n `"`$PREVIOUS_RELEASE`" ] && [ -d `"`$PREVIOUS_RELEASE`" ]; then cd `$PREVIOUS_RELEASE; $productionComposeCommand up -d --remove-orphans redis; wait_for_redis; $productionComposeCommand up -d --wait --wait-timeout 600 --remove-orphans `$WAITED_SERVICES; $productionComposeCommand up -d `$BACKGROUND_SERVICES; $productionComposeCommand up -d `$MONITOR_SERVICE; ln -sfn `$PREVIOUS_RELEASE /opt/skn27-pilot/current; fi; exit `$status; }",
            ...
            "for attempt in `$(seq 1 30); do ! ss -ltnp | grep -E '(:80|:443)' >/dev/null && break; if [ `$attempt -eq 30 ]; then echo 'Caddy ports 80/443 were not released for host-network cutover.' >&2; exit 78; fi; sleep 1; done",
            "cd `$RELEASE_DIR",
            "promote_operational_evidence",
            "$productionComposeCommand pull",
            "PILOT_BACKEND_IP='172.31.0.11' $productionComposeCommand run --rm --no-deps backend python backend/manage.py help smoke_non_dl_analysis_reporting_pipeline >/dev/null",
            "PILOT_BACKEND_IP='172.31.0.11' $productionComposeCommand run --rm --no-deps backend python backend/manage.py help smoke_supervisor_conversation_runtime >/dev/null",
            "PILOT_BACKEND_IP='172.31.0.11' $productionComposeCommand run --rm --no-deps backend python backend/manage.py migrate --check",
            "$productionComposeCommand up -d --remove-orphans redis",
            "wait_for_redis",
            "$productionComposeCommand up -d --wait --wait-timeout 600 --remove-orphans `$WAITED_SERVICES",
