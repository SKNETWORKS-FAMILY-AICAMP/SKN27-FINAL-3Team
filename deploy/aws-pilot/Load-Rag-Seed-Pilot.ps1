...
$runnerCommands = @(
    ...
    "$stageComposeCommand --profile seed run --rm --no-deps -v `$RAG_DIR:/run/production-rag-seed:ro rag-loader python backend/manage.py load_production_rag_seed --manifest /run/production-rag-seed/$RagSeedManifestRelativePath --skip-legal-schema --format json",
