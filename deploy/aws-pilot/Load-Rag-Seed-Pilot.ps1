...
$commands = @(
    ...
    "$stageComposeCommand run --rm --no-deps rag-loader python backend/manage.py help verify_production_rag_seed_manifest >/dev/null",
    "$stageComposeCommand run --rm --no-deps rag-loader python backend/manage.py help load_review_case_pgvector_seed >/dev/null",
    "$stageComposeCommand run --rm --no-deps rag-loader python backend/manage.py help load_production_rag_seed >/dev/null",
    "$stageComposeCommand run --rm --no-deps rag-loader python backend/manage.py help load_legal_graph_seed >/dev/null",
    "RELEASE_STATE_FILE=`$TARGET_RELEASE/.production-rag-seed.complete",
    "test ! -e `$RELEASE_STATE_FILE",
    "load_failed() { status=`$?; trap - ERR EXIT; cd `$TARGET_RELEASE 2>/dev/null || true; $stageComposeCommand down --remove-orphans >/dev/null 2>&1 || true; docker volume rm '${stageProjectName}_redis_data' '${stageProjectName}_clamav_data' '${stageProjectName}_law_neo4j_data' '${stageProjectName}_law_neo4j_logs' >/dev/null 2>&1 || true; rm -rf -- `$TARGET_RELEASE; exit `$status; }",
    ...
    "$stageComposeCommand run --rm --no-deps -v `$RAG_DIR:/run/production-rag-seed:ro rag-loader python backend/manage.py verify_production_rag_seed_manifest --manifest /run/production-rag-seed/$RagSeedManifestRelativePath --format json",
    "$stageComposeCommand run --rm --no-deps rag-loader python -c `"from pathlib import Path; production=Path('backend/chatbot/management/commands/load_production_rag_seed.py').read_text(); legal=Path('backend/chatbot/management/commands/load_legal_rag_pgvector.py').read_text(); assert 'load_and_validate_rag_seed_manifest' in production; assert 'load_legal_rag_pgvector' in production; assert 'transaction.atomic' in legal`"",
    "$stageComposeCommand run --rm --no-deps -v `$RAG_DIR:/run/production-rag-seed:ro rag-loader python backend/manage.py load_review_case_pgvector_seed --manifest /run/production-rag-seed/$RagSeedManifestRelativePath --replace --allow-paid-provider-call --format json",
    "$stageComposeCommand run --rm --no-deps -v `$RAG_DIR:/run/production-rag-seed:ro rag-loader python backend/manage.py load_production_rag_seed --manifest /run/production-rag-seed/$RagSeedManifestRelativePath --skip-legal-schema --format json",
    ...
    "$stageComposeCommand run --rm --no-deps -v `$RAG_DIR:/run/production-rag-seed:ro rag-loader python backend/manage.py load_legal_graph_seed --manifest /run/production-rag-seed/$RagSeedManifestRelativePath --dataset-version `"`$LEGAL_DATASET_VERSION`" --replace --format json",
    "cleanup_rag_seed",
    "trap - EXIT",
    "$stageComposeCommand run --rm --no-deps rag-loader python backend/manage.py verify_legal_graph_readiness --format json",
    "$stageComposeCommand run --rm --no-deps rag-loader python backend/manage.py smoke_law_ground_search --require-results --format json",
    "$stageComposeCommand run --rm --no-deps rag-loader python backend/manage.py verify_pgvector_rag_readiness --format json",
    "$stageComposeCommand run --rm --no-deps rag-loader python backend/manage.py smoke_text_ml_case_search --require-pgvector --require-results --format json",
