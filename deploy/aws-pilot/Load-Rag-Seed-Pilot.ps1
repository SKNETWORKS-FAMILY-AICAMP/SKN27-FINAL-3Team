...
$commands = @(
    ...
    "load_failed() { status=`$?; trap - ERR EXIT; cd `$TARGET_RELEASE 2>/dev/null || true; $stageComposeCommand down --remove-orphans >/dev/null 2>&1 || true; docker volume rm '${stageProjectName}_redis_data' '${stageProjectName}_elasticsearch_data' '${stageProjectName}_clamav_data' >/dev/null 2>&1 || true; rm -rf -- `$TARGET_RELEASE; exit `$status; }",
    ...
    "$stageComposeCommand run --rm --no-deps -v `$RAG_DIR:/run/production-rag-seed:ro backend python backend/manage.py load_production_rag_seed --manifest /run/production-rag-seed/$RagSeedManifestRelativePath --replace-legal --recreate-es --format json",
    "cleanup_rag_seed",
    "trap - EXIT",
    "$stageComposeCommand run --rm --no-deps backend python backend/manage.py smoke_law_ground_search --require-results --format json",
    "$stageComposeCommand run --rm --no-deps backend python backend/manage.py smoke_text_ml_case_search --require-es --require-results --format json",
