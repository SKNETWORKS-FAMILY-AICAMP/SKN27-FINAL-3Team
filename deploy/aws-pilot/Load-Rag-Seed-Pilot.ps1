...
$commands = @(
    ...
    "$stageComposeCommand run --rm --no-deps backend python backend/manage.py help load_legal_graph_seed >/dev/null",
    ...
    "$stageComposeCommand run --rm --no-deps -v `$RAG_DIR:/run/production-rag-seed:ro backend python backend/manage.py load_legal_graph_seed --manifest /run/production-rag-seed/$RagSeedManifestRelativePath --dataset-version `"`$LEGAL_DATASET_VERSION`" --replace --format json",
    "cleanup_rag_seed",
    "trap - EXIT",
    "$stageComposeCommand run --rm --no-deps backend python backend/manage.py verify_legal_graph_readiness --format json",
    "$stageComposeCommand run --rm --no-deps backend python backend/manage.py smoke_law_ground_search --require-results --format json",
    "$stageComposeCommand run --rm --no-deps backend python backend/manage.py verify_pgvector_rag_readiness --format json",
    "$stageComposeCommand run --rm --no-deps backend python backend/manage.py smoke_text_ml_case_search --require-pgvector --require-results --format json",
