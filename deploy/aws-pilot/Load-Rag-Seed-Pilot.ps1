...
$commands = @(
    ...
    "load_failed() { status=`$?; trap - ERR EXIT; cd `$TARGET_RELEASE 2>/dev/null || true; $stageComposeCommand down --remove-orphans >/dev/null 2>&1 || true; docker volume rm '${stageProjectName}_redis_data' '${stageProjectName}_clamav_data' >/dev/null 2>&1 || true; rm -rf -- `$TARGET_RELEASE; exit `$status; }",
