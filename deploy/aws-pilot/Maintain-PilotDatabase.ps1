...
try {
    & aws ec2 replace-iam-instance-profile-association --region $region --association-id $associationId --iam-instance-profile "Name=$maintenanceProfile" --no-cli-pager | Out-Null
    ...
    $commands = @(
        ...
        "python3 -c 'import json,sys; b=open(sys.argv[1]).read().splitlines(); m=json.load(open(sys.argv[2])); out=[x for x in b if not x.startswith(`"POSTGRES_`") and not x.startswith(`"DJANGO_DATABASE_ENGINE=`") and not x.startswith(`"PGSSLMODE=`")]; out += [`"DJANGO_DATABASE_ENGINE=postgres`",`"PGSSLMODE=require`",f`"POSTGRES_HOST={sys.argv[4]}`",f`"POSTGRES_PORT={sys.argv[5]}`",f`"POSTGRES_DB={sys.argv[6]}`",f`"POSTGRES_USER={m[`"username`"]}`",f`"POSTGRES_PASSWORD={m[`"password`"]}`"]; open(sys.argv[3],`"w`").write(`"\\n`".join(out)+`"\\n`")' `$WORK/base.env `$WORK/master.json `$WORK/master.env '$databaseHost' '$databasePort' '$databaseName'",
        "python3 -c 'import json,sys; m=json.load(open(sys.argv[1])); out=[f`"PGHOST={sys.argv[3]}`",f`"PGPORT={sys.argv[4]}`",f`"PGDATABASE={sys.argv[5]}`",f`"PGUSER={m[`"username`"]}`",f`"PGPASSWORD={m[`"password`"]}`",`"PGSSLMODE=require`"] ; open(sys.argv[2],`"w`").write(`"\\n`".join(out)+`"\\n`")' `$WORK/master.json `$WORK/libpq.env '$databaseHost' '$databasePort' '$databaseName'",
