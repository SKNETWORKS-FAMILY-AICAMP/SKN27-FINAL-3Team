...
try {
    ...
    $commands = @(
        ...
        "docker run --rm '${backendRepository}:${maintenanceImageTag}' cat /app/etl/fault_cases/src/traffic_precedents/precedent_db_loading/schema.sql | docker run --rm -i --env-file `$WORK/libpq.env '$postgresMaintenanceImageRef' psql -v ON_ERROR_STOP=1",
