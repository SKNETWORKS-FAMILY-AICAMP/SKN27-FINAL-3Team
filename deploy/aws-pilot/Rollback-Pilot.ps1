...
$commands = @(
    ...
    "python3 -c 'import json,re; m=json.load(open(`"deployment-manifest.json`")); assert re.search(`"@sha256:[0-9a-f]{64}$`",m[`"NginxImageRef`"]); assert re.search(`"@sha256:[0-9a-f]{64}$`",m[`"ElasticsearchImageRef`"]); assert re.search(`"@sha256:[0-9a-f]{64}$`",m[`"PostgresMaintenanceImageRef`"]);'",
    "grep -E '^(APP_DOMAIN|ACME_EMAIL)=' .runtime.env > .edge.env",
    "grep -E '^ELASTIC_PASSWORD=' .runtime.env > .elasticsearch.env",
    "chmod 0600 .runtime.env .compose.env .edge.env .elasticsearch.env",
