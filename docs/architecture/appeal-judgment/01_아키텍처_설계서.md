...
| `merit_classification_node` (MG) | 고정 참조 법조문을 컨텍스트로 LLM 단일 호출. **(v4.4) `notice_stage`에 따라서만 분기한다** — 142조·160조4항1호 본문(도난 포함)·제7조를 위반유형 무관하게 항상 함께 주입하고, 1차 고지서면 제14조를 추가한다(§9-2 정정). `law_code` 기반 주정차/비주정차 분기는 더 이상 하지 않는다 | `user_appeal_reason`, `notice_stage`, LDB 참조 조문 | `merit` |
...
### 9-2. 시행규칙 제142조 적용범위 문제 — v3.6 "해소"는 폐기, v4.4에서 재정정

> **⚠️ 아래 v3.6 결정은 폐기됐다.** `law160-budeuk-hansayu-scope-analysis2.md`에서 §160④
> ("제3항의 규정에 불구하고...")가 §160③ **전체**(1호 주정차 + 2호 비주정차 일반)에 적용되는
> 예외임이 확인되면서, "142조=주정차 전용"이라는 전제 자체가 틀렸다는 게 드러났다. 이 절은
> v3.6 당시의 판단 과정을 이력으로 남기고, 맨 아래 "v4.4 재정정"이 최신 결론이다.
...
**결정 (v3.6, 폐기됨)**: 비주정차 위반(과속·신호위반 등 무인단속)에는 **질서위반행위규제법 제7조**
...
**v4.4 재정정 — 최종 결론**: 142조는 §160④1호("부득이한 사유")의 정의 조항이고, §160④는 §160③
전체(1호 주정차 + 2호 비주정차 일반)에 적용되는 예외라, 142조 문언 자체에도 주정차 한정 문구가
없다. 따라서 `law_code` 기반 주정차/비주정차 판별·분기는 필요 없고, `MG`는 `notice_stage`에
따라서만 분기한다 — 142조·160조4항1호 본문·제7조를 위반유형 무관 공통으로 항상 함께 주입하고,
1차 고지서면 제14조를 추가한다. 제7조는 142조를 "대체"하는 게 아니라, 142조 6개 항목에
해당하지 않는 경우의 보조·병존 원칙으로 항상 병행 적용한다. `is_parking_violation()`·
`PARKING_VIOLATION_LAW_CODE_PATTERN`은 라우팅 신호 자체가 불필요해져 삭제했다. 상세 매핑표는
`law160-budeuk-hansayu-scope-analysis2.md` §6, DATA-003 §5(갱신 필요) 참고.
...
제160조4항1호 본문도 함께 주입한다 (DATA-003 §5 갱신). **(v4.4 정정)** 이 본문은 위반유형 무관
공통 조문이라, 애초에 "주정차 위반 케이스에만" 넣는다는 구분 자체가 없다 — 비주정차 위반도
제7조와 함께 항상 포함된다.
...
| `merit_gate.py` | `merit_classification_node` (MG) — `notice_stage`별 공통 참조 법조문(위반유형 무관) → LLM 단일 호출(§9-2 v4.4 재정정, §9-6) | `law_refs.py`의 상수를 컨텍스트로 주입, `law_code` 라우팅 없음 |
...
| unit | `TestLawRefs` | 3 | **(v4.4 갱신)** notice_stage별 공통 MG 컨텍스트 조립 — 위반유형 무관하게 142조·160조·제7조 항상 포함 확인 (§9-2 재정정, §9-6) |
...
| integration | `TestSuccessBranch` | 5 | 도난/카테고리 A·B·C 조합별 정상 완료 경로, **(v4.4 갱신)** 비주정차 위반도 142조+제7조 공통 컨텍스트로 판단하는지 확인 |
