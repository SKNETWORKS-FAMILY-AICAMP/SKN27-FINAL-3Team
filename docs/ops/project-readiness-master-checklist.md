...
- [~] 채팅 메시지·에이전트 실행 진입점에서 입력 보호를 부수 효과보다 앞에 배치하고 안전한 4xx로 변환 — #219
- [ ] 보고서·다운로드·운영 로그까지 포함한 개인정보 노출 회귀 테스트
- [ ] 프롬프트 인젝션과 비신뢰 OCR/RAG 자료가 시스템 지시·도구 호출 조건으로 작동하지 않도록 하는 경계

### A-2. 객관적 사실과 주관적 주장 분리

- [ ] 사건 입력을 검증 가능한 사실, 사용자 주장, 미확인·추정으로 구조화
- [ ] Supervisor 입력 스키마에 `fact`, `claim`, `unknown`, `evidence_source` 구분 추가
- [ ] 에이전트 호출 전 근거 없는 주장을 검색 조건·과실 판단 근거로 단정 사용하지 않도록 가드레일 추가
- [ ] 응답과 보고서에서 사용자 진술, 첨부자료 확인, 추가 확인 필요를 구분 표기
- [ ] 증거 부족 시 과실 판단 대신 필요한 자료를 요청하는 역질문 연결
- [ ] 주관적 진술·상충 진술·증거 없는 진술 회귀 테스트
- [ ] Supervisor 입력 스키마 → 역질문 → 호출 계획 → 에이전트 결과 취합 → 사용자 응답의 E2E 계약 테스트
...
- [ ] 리포트 목록·생성·상세·다운로드 API 계약
- [ ] 이의신청서 초안, 사실관계 정리, 보험사 제출 자료를 문서 종류로 분리
- [ ] 문서 생성 전 사용자 최종 확인: 사실관계, 관할기관, 기한, 첨부자료
- [ ] DOCX/PDF 한글 렌더링, 개인정보 마스킹, 권한 검증
...
1. [~] #219 채팅·에이전트 실행 입력 보호 경계
2. [ ] 사실·주장·미확인 정보 분리 가드레일
3. [ ] 서비스 범위·판단 불가 사례·기한 강조 정책
4. [ ] 사건 메모리·역질문·채팅 세션 계약
5. [ ] 리포트 API 및 문서 E2E
6. [ ] 근거 최신성 및 검색 품질 평가
