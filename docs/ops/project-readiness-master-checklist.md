...
- [~] 사건 입력을 검증 가능한 사실, 사용자 주장, 미확인·추정으로 구조화 — #221
- [~] Supervisor 입력 스키마에 `fact`, `claim`, `unknown`, `evidence_source` 구분 추가 — #221
- [~] 에이전트 호출 전 근거 없는 주장을 검색 조건·과실 판단 근거로 단정 사용하지 않도록 가드레일 추가 — #221
- [~] 응답과 보고서에서 사용자 진술, 첨부자료 확인, 추가 확인 필요를 구분 표기 — #221
- [~] 증거 부족 시 과실 판단 대신 필요한 자료를 요청하는 역질문 연결 — #221
- [~] 주관적 진술·상충 진술·증거 없는 진술 회귀 테스트 — #221
