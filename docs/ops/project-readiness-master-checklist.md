...
- [~] 보고서·다운로드 개인정보 노출 회귀 테스트 — #226 (운영 로그 경계는 별도 작업)
...
- [~] #224 채팅 세션 기반 역질문 상태 저장·서버 우선 복원 계약
...
- [~] 리포트 목록·상세·다운로드 API 계약 및 문서 E2E — #226 (생성 POST는 deferred 유지)
...
- [~] 리포트 조회·다운로드 API 계약 — #226 (생성 POST는 deferred 유지)
- [ ] 채팅 세션·메시지·저장 API 계약
- [ ] 마이페이지 API 계약
- [ ] 히스토리 API 계약
- [ ] 에이전트 노드 API 계약
...
5. [~] #226 리포트 조회·다운로드 API 및 문서 E2E (생성 POST deferred)
6. [ ] 근거 최신성 및 검색 품질 평가
