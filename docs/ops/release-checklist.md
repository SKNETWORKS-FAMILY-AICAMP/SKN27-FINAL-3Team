...
- [x] 자동 테스트가 통과했다. — PR #300 병합 전 CI 사용자 확인, 브라우저 QA 집중 회귀 `48 passed`, Ruff 통과, Vite production build 성공
...
- [~] 법령 데이터 최신성 게이트가 자동화되어 있다. — source별 run summary와 stale·missing·failed 차단 CLI·runbook 구현, 운영 승인 임계값·운영 DB 실증·CloudWatch 연결은 남음
