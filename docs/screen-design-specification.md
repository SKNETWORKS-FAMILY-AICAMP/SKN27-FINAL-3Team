...
| 최종 업데이트 | 2026-06-22 |
| 문서 버전 | v0.3 |
| 작성 기준 | 요구사항 정의서 v0.4, 통합 화면정의서 이미지, Notion 작업 트래커 확인 결과, `#12` 화면 흐름 이슈, 2026-06-19/2026-06-22 회의 정리 |
| 참조 화면 | `docs/assets/screen-design/final-screen-plan-complete-updated.png`, `docs/assets/screen-design/mypage-updated.png`, `docs/assets/screen-design/report-pages-updated.png`, `app/screen-design-mvp-flow.html`, `docs/screen-design-ui-ux-flow-guide.md` |
...
### 1.1 2026-06-22 업데이트 목적

2026-06-22 업데이트는 오늘 발표에서 설명해야 하는 로그인/서비스 설명 진입, 챗봇, Supervisor 분기, 결과/리포트 진입 흐름을 화면설계서 본문에 연결하기 위한 것이다.

기존 화면정의서 이미지에 이미 반영된 마이페이지, 챗봇, 이력, 리포트 계열 화면은 유지한다. 다만 `#12 docs-mvp-screen-and-process-flows` 기준으로 발표자가 사용자 사용 흐름을 설명할 수 있도록 아래 항목을 보강한다.

| 보강 항목 | 반영 방식 | 상태 |
|---|---|---|
| 로그인/서비스 설명 진입 | 별도 홈 화면이 아니라 로그인과 서비스 설명을 포함한 진입 화면으로 정리 | 서비스 UI 반영 |
| Google 로그인 흐름 | 로그인 완료 후 챗봇 화면으로 바로 이동하는 구조로 정리 | 서비스 UI 반영 |
| 챗봇 입력 유형 | 고지서, 사고 설명, 사진·영상, 법령 질문으로 분리 | `#29`와 연결 필요 |
| Supervisor 분기 | OCR, 법률 근거, 과실비율, Vision, 이의신청서 노드 후보로 표현 | `#22`, `#29`와 연결 필요 |
| 결과/리포트 진입 | 과태료·범칙금 결과와 과실비율 결과의 후속 행동 분리 | 서비스 UI 반영 |
| HTML 산출물 | `app/screen-design-mvp-flow.html`은 사용자 서비스 화면으로 구성하고, 설명은 `docs/screen-design-ui-ux-flow-guide.md`로 분리 | 반영 |

본 업데이트에서 API endpoint, 모델명, 샘플 검증 결과, 법률 판단 기준은 새로 확정하지 않는다. 문서에 근거가 부족한 항목은 `검증 필요`로 남긴다.
...
|---|---|---|---|---|
| `UI-ENTRY-001` | 로그인/서비스 설명 진입 | 별도 홈 화면 없이 서비스 목적, Google 로그인, 챗봇 바로 시작 CTA를 제공 | 포함 | `screen-design-mvp-flow.html` |
| `UI-AUTH-001` | Google 로그인 | Google 로그인, 신규 사용자 약관 동의 또는 가입 완료 후 챗봇 진입 | 포함 | `screen-design-mvp-flow.html` |
...
`UI-ENTRY-001`, `UI-AUTH-001`은 2026-06-22 사용자 흐름 정리를 위한 후보 ID다. 별도 홈 화면을 만들지 않고 로그인/서비스 설명 진입 화면에서 바로 챗봇으로 넘어가는 구조를 기준으로 한다. 요구사항 정의서 또는 최종 화면정의서에서 별도 ID가 확정되면 해당 ID로 치환한다.
...
| 영역 | 표시 내용 | 동작 |
|---|---|---|
| 로고 | 서비스명 `교통분쟁 AI` | 클릭 시 로그인/서비스 설명 진입 영역으로 이동 |
| 상담 시작 | `챗봇 바로 시작` 또는 `진단하기` | 별도 홈 화면 없이 `UI-Ai-01` 챗봇 화면으로 이동 |
...
| 번호 | 영역 | 표시 내용 | 요구사항 매핑 |
|---|---|---|---|
| 1 | 상단 내비게이션 | 로고, 챗봇 바로 시작, 마이페이지 | 공통 |
...
| 경로 | 설명 |
|---|---|
| 로그인/서비스 설명 진입 화면의 `챗봇 바로 시작` | AI 교통 상담 챗봇 메인 화면으로 이동 |
...
## 18. 2026-06-22 사용자 UI/UX 흐름

### 18.1 시연 기준

오늘 발표에서는 개별 화면의 세부 구현보다 사용자가 어떤 순서로 서비스를 쓰고, 화면이 Agent/Supervisor 흐름과 어떻게 연결되는지를 설명한다. 별도 홈 화면은 만들지 않고, 로그인과 서비스 설명을 포함한 진입 화면에서 챗봇으로 바로 넘어가는 흐름을 기준으로 한다.

| 기준 | 내용 |
|---|---|
| 발표 목표 | 로그인/서비스 설명 진입 화면에서 시작해 챗봇 입력, Supervisor 분기, 결과/리포트 확인까지의 MVP 흐름 설명 |
| 기준 이슈 | `#12 docs-mvp-screen-and-process-flows`, `#22 feat-agent-result-schema-and-rag-contract`, `#29 feat-supervisor-chatbot-routing`, `#40 test-cross-mvp-integration-scenarios` |
| 확정 범위 | 화면 진입 순서, 화면별 사용자 액션, 결과 화면의 정보 구조 |
| 검증 필요 | API endpoint, 모델 확정, 샘플 검증 결과, 법률 근거 검색 호출 시점의 최종 확정 |
| 산출물 | `docs/screen-design-specification.md`, `app/screen-design-mvp-flow.html`, `docs/screen-design-ui-ux-flow-guide.md` |

### 18.2 핵심 사용자 여정

| 순서 | 화면 | 사용자 목적 | 주요 액션 | 다음 화면 |
|---:|---|---|---|---|
| 1 | `UI-ENTRY-001` 로그인/서비스 설명 진입 | 서비스가 어떤 문제를 해결하는지 이해하고 바로 상담을 시작한다. | 상담 유형 확인, `Google 로그인`, `챗봇 바로 시작` 클릭 | `UI-AUTH-001` 또는 `UI-Ai-01` |
| 2 | `UI-AUTH-001` Google 로그인 | 본인 이력과 리포트를 저장할 수 있는 상태로 진입한다. | Google 로그인, 신규 사용자는 약관 동의 | `UI-Ai-01` |
| 3 | `UI-Ai-01` AI 교통 상담 챗봇 | 고지서, 사고 설명, 사진·영상, 법령 질문을 입력한다. | 텍스트 입력, 파일 업로드, 추천 질문 선택 | Supervisor 분기 |
| 4 | Supervisor 분기 | 입력 유형에 맞는 Agent 노드를 호출한다. | 사용자는 직접 노드를 선택하지 않고 화면은 진행 상태를 표시한다. | 결과 카드 |
| 5 | 결과 카드 | 분석 요약, 근거, 한계, 다음 행동을 확인한다. | 상세보기, 근거 보기, 추가 업로드, 리포트 생성 | 상세/리포트 화면 |
| 6 | 리포트/이의신청서 | 결과를 문서화하거나 후속 행동을 준비한다. | PDF 저장, DOCX 저장, 이의신청서 초안 확인 | `UI-MY-001`, `UI-REPORT-*` |

### 18.3 UI/UX 흐름 원칙

| 원칙 | 화면 반영 |
|---|---|
| 사용자는 Agent 이름보다 문제 유형을 먼저 본다. | 로그인/서비스 설명 진입 화면과 챗봇에서는 `과태료·범칙금`, `과실비율`, `사진·영상`, `법령 근거`처럼 사용자의 언어로 진입점을 제공한다. |
| Supervisor는 내부 처리 흐름으로 설명한다. | 챗봇 화면에는 `고지서 확인 중`, `법률 근거 확인 중`, `유사 사례 확인 중` 같은 진행 상태로 노출한다. |
| 결과는 단정하지 않는다. | 과실비율 수치 확정, 법률 판단 확정, 제출 성공 보장 표현을 쓰지 않고 근거와 한계를 함께 표시한다. |
| 자료 부족은 실패가 아니라 다음 행동으로 연결한다. | OCR 필드 누락, 영상 품질 낮음, 근거 부족은 추가 질문/추가 업로드 CTA로 연결한다. |
| HTML은 사용자 서비스 화면으로 보여준다. | `screen-design-mvp-flow.html`은 정적 HTML이지만 화면 안에는 발표용 설명 대신 실제 사용 화면의 UI/UX 흐름을 배치한다. |
| 홈은 독립 화면으로 분리하지 않는다. | 로그인/서비스 설명 진입 화면에서 서비스 목적과 주요 상담 유형을 보여주고, 로그인 또는 챗봇 바로 시작으로 연결한다. |

### 18.4 입력 유형별 Supervisor 분기 초안

| 사용자 입력 | 우선 호출 노드 | 필요한 화면 상태 | 다음 행동 |
|---|---|---|---|
| 고지서 이미지 또는 OCR 텍스트 | 고지서 OCR·과태료/범칙금 분석 노드 | OCR 상태, 필수 필드, 누락 필드, 신뢰도 표시 | 필드 부족 시 추가 질문, 충분하면 법률 근거 검토 |
| 법령 근거 질문 | 법률 근거 검색 노드 | 관련 법령, 조문, 적용 한계, 최신성 표시 | 근거 요약 후 리포트 또는 이의신청서 초안 연결 |
| 사고 설명 또는 경위서 | 텍스트 ML/판례·사례 검색 노드 | 사고 요약, 쟁점 태그, 유사 사례 후보 표시 | 과실비율 상세 또는 유사 사례 목록 연결 |
| 사진·영상 | Vision 분석 노드 | key frame, 장면 요약, confidence, 품질 이슈 표시 | 텍스트 분석 결과와 병합 또는 추가 자료 요청 |
| 이의신청서 요청 | 이의신청서 생성/리포트 노드 | 분석 결과, 법률 근거, 필요 증거, 면책 문구 표시 | 초안 생성, 다운로드, 추가 입력 요청 |

### 18.5 발표에서 설명할 화면 순서

| 발표 순서 | 설명 포인트 | 보여줄 산출물 |
|---:|---|---|
| 1 | 로그인/서비스 설명 진입 화면에서 사용자가 문제 유형을 확인하고 로그인 또는 챗봇 바로 시작을 선택한다. | `screen-design-mvp-flow.html`의 진입 영역 |
| 2 | 챗봇은 하나의 화면이지만 입력 유형별로 다른 처리 흐름을 탄다. | 챗봇 입력 영역 |
| 3 | Supervisor는 내부에서 OCR, 법률, 과실비율, Vision, 이의신청서 노드를 조합한다. | Supervisor 노드 영역 |
| 4 | 결과 화면은 과태료·범칙금과 과실비율을 분리해 보여준다. | 결과 카드 영역 |
| 5 | 마이페이지, 과거 이력, 리포트 목록으로 저장된 결과를 다시 확인한다. | 이력/리포트 연결 영역 |
| 6 | 미확정 항목은 `검증 필요`로 분리해 중간 발표 이후 구현 기준을 명확히 한다. | 검증 필요 항목 영역 |

### 18.6 오늘 기준 완료 체크리스트

| 항목 | 완료 기준 | 상태 |
|---|---|---|
| 화면설계서 업데이트 | 로그인/서비스 설명 진입, 챗봇, Supervisor, 결과/리포트 흐름이 본문에 명시됨 | 반영 |
| HTML 산출물 | 한글 UTF-8 정적 HTML로 실제 서비스 사용 흐름을 시연할 수 있음 | 반영 |
| 화면 ID 정리 | 기존 ID와 발표용 후보 ID가 구분됨 | 반영 |
| API 미확정 처리 | endpoint와 schema 확정 전 항목을 `검증 필요`로 표시 | 반영 |
| 모델/법률 판단 단정 방지 | 화면 문구에서 확정 판단처럼 보이지 않도록 주의 문구 포함 | 반영 |

### 18.7 남은 리스크

| 리스크 | 영향 | 다음 액션 |
|---|---|---|
| `UI-ENTRY-001`, `UI-AUTH-001`의 최종 요구사항 ID 미확정 | 구현 시 ID가 바뀔 수 있음 | 요구사항 정의서에 최종 ID 반영 |
| 고지서 샘플 5~6개 검증 결과 미반영 | 과태료·범칙금 결과 화면의 필드 우선순위가 바뀔 수 있음 | `#28` 결과 반영 |
| 법률 근거 검색 호출 시점 미확정 | Supervisor 흐름과 챗봇 진행 상태 문구가 바뀔 수 있음 | `#26`, `#29` 회의 결정 반영 |
| Vision/DL 모델 확정 전 | 사진·영상 화면의 confidence와 품질 이슈 표현이 바뀔 수 있음 | `#36`, `#38` 결과 반영 |
| API 정의서 미완성 | 로딩, 오류, 빈 상태의 실제 구현 기준이 추상적일 수 있음 | API 명세서 작성 후 재검토 |
