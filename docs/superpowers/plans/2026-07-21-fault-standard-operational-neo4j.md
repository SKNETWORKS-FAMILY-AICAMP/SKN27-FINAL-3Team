...
- [x] **Step 1: Write failing tests** for deterministic JSONL output, SHA-256 manifest fields, and rejection of unsafe namespace labels.
- [x] **Step 2: Run `python -m pytest -q etl/fault_cases/rag_runtime/database/tests/test_graph_export.py`** and confirm the tests fail because the module does not exist.
- [x] **Step 3: Implement the exporter** with safe identifier validation, explicit node/relationship properties, stable key ordering, UTF-8 JSONL, and manifest counts/hashes.
- [x] **Step 4: Run the focused tests** and confirm all pass.
- [x] **Step 5: Commit** `feat: add auditable Neo4j graph exporter`.
...
- [x] **Step 1:** Read the current container credentials without printing them and run the exporter against `bolt://localhost:7688`.
- [x] **Step 2:** Write V7 to `C:/dev/project/SKN27-RAG-rescue/etl/fault_cases/HISTORY_LOCAL/neo4j_archives/complete30_v7/`.
- [x] **Step 3:** Write V9 staging input to `C:/dev/project/SKN27-RAG-rescue/etl/fault_cases/HISTORY_LOCAL/neo4j_archives/complete30_v9_source/`.
- [x] **Step 4:** Verify manifests show exactly 1,718/1,441 and 7,815/13,196; stop if either differs.
...
- [x] **Step 1:** Write failing tests for V9 label transformation, removal of `Complete30V9`/`V9Import`, metadata insertion, and preservation of role labels/properties.
- [x] **Step 2:** Run the focused tests and confirm the intended failures.
- [x] **Step 3:** Implement fresh-graph import with a `FaultStandardOperational` uniqueness constraint on `source_legacy_element_id`, validator-enforced uniqueness for `Rule.rule_id` (Neo4j Community does not accept a multi-label uniqueness constraint), and relationship MERGE by source relationship ID.
- [x] **Step 4:** Run focused tests and confirm all pass.
- [x] **Step 5:** Commit `feat: add fault standard operational graph importer`.
...
- [x] **Step 1:** Write failing tests for expected constants, required query label generation, and PASS/FAIL report behavior.
- [x] **Step 2:** Run focused tests and confirm failures.
- [x] **Step 3:** Implement validator checks for counts, forbidden labels, duplicate/missing IDs, required Rule paths, missing `record_json`, isolated nodes, and constraints.
- [x] **Step 4:** Update Runtime queries to use `OPERATIONAL_LABEL` and rename only the adapter module’s public semantics; retain evaluation folder names.
- [x] **Step 5:** Run focused tests, `compileall`, and the existing RAG test suite.
- [x] **Step 6:** Commit `feat: validate and query operational Neo4j graph`.
...
- [x] **Step 1:** Start a temporary Neo4j 5 Community container with an explicit `fault_standard_neo4j_operational_next` volume and a temporary host port.
- [x] **Step 2:** Import only `complete30_v9_source` into the temporary database.
- [x] **Step 3:** Run the structural validator and save its JSON report beside the rescue archive.
- [x] **Step 4:** Run Runtime smoke queries against the temporary database with the operational label.
- [x] **Step 5:** Stop and preserve the temporary container if any check fails; do not touch the current container.
...
- [x] **Step 1:** Capture a final current-container and volume inventory; confirm V7 archive manifest and V9 staging manifest hashes.
- [x] **Step 2:** Stop the current `fault-standard-neo4j` service and retain its old volume under an explicit archive name.
- [x] **Step 3:** Attach the validated operational volume to the service name `fault-standard-neo4j`.
- [x] **Step 4:** Run the validator, Runtime smoke test, and targeted RAG tests after cutover.
- [x] **Step 5:** Leave the old volume recoverable until the user confirms cleanup; do not delete it in the same step as cutover.
- [x] **Step 6:** Commit only repository changes and record the cutover report; generated DB volumes remain outside Git.

### Task 7: Final verification

- [x] **Step 1:** Run `python -B -m pytest -q -p no:cacheprovider etl/fault_cases/rag_runtime etl/fault_cases/src --disable-warnings`.
- [x] **Step 2:** Run `python -B -m compileall -q etl/fault_cases/rag_runtime etl/fault_cases/src`.
- [x] **Step 3:** Run the operational Neo4j validator and inspect its full JSON report.
- [x] **Step 4:** Confirm `git status --short` contains only intended repository changes and no generated graph backup.
