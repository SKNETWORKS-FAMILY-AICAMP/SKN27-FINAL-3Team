...
- 인앱 브라우저가 이 세션의 `127.0.0.1` 제어를 정책상 거부했다.
  프로덕션 빌드와 UI 계약 테스트는 통과했지만 실제 클릭 스모크는 사람이
  릴리스 직전에 수행해야 한다.
...
6. 데스크톱·모바일에서 홈 → 첨부 → 분류/OCR 확인 → 결과 → 리포트
   다운로드 브라우저 스모크
...
GitHub 커넥터의 자동 승인 모델 오류로 이 세션에서 이슈 생성이 거부되었다.
우회 생성하지 않고 다음 두 제목과 범위를 인계한다.

1. `[P1] 구조화 사건 입력과 장기 대화 메모리 고도화`
   - 구조화 입력 UI, 서버 우선 사건 상태, 장기 대화 압축 시
     사실·주장·출처·미확인·기한 보존, 모바일·접근성 검증
2. `[P1] 운영 데이터 최신성·관측·재현성 증적 보강`
