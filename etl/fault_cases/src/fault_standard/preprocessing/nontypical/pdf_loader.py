...
def load_pdf_pages(pdf_path: Path) -> Tuple[List[PageText], Dict[str, Any]]:
    ...
    # 표 구조가 있는 PDF라서 pdfplumber를 먼저 시도합니다.
    try:
        return load_with_pdfplumber(pdf_path)

    # pdfplumber가 실패하면 PyMuPDF로 다시 읽습니다.
    except Exception as first_error:
        pages, report = load_with_pymupdf(pdf_path)
...
def load_with_pdfplumber(pdf_path: Path) -> Tuple[List[PageText], Dict[str, Any]]:
    ...
    with pdfplumber.open(str(pdf_path)) as pdf:
        ...
        for idx, page in enumerate(pdf.pages, start=1):
            try:
                # x/y tolerance를 주어 표 내부 텍스트를 읽습니다.
                raw = page.extract_text(x_tolerance=1, y_tolerance=3) or ""
                error = None

            except Exception as exc:
                # 특정 페이지가 실패해도 전체 처리를 멈추지 않습니다.
                raw = ""
                error = str(exc)
