...
from .retriever import compact_result, search_query


def run_samples(top_k: int, include_text: bool = False) -> dict[str, Any]:
    query_reports = []
    for query_info in SAMPLE_QUERIES:
        results = search_query(query=query_info["query"], top_k=top_k)
    ...
    report_path = POSTGRES_EXPORT_ROOT / PGVECTOR_SEARCH_SETTINGS.sample_report_name
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run review_case pgvector baseline sample queries.")
