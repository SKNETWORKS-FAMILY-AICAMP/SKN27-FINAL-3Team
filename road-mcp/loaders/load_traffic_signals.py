...
SOURCE_NAME = "traffic_signals"


def build_extra_params() -> dict[str, str]:
    """신호등 API에 추가로 넘길 파라미터를 만든다."""
    return {"type": "json"}


async def preview() -> dict:
    """신호등 API를 1건 호출하고 응답 샘플을 저장한다."""
    settings = get_settings()
    return await preview_public_data_api(
        source_name=SOURCE_NAME,
        api_url=settings.traffic_signals_api_url,
...
async def collect_raw_pages(max_pages: int | None = None, start_page: int = 1) -> None:
    """신호등 API 원본 응답을 페이지별 스냅샷으로 저장한다."""
    settings = get_settings()
...
def parse_args() -> argparse.Namespace:
    """명령행 옵션을 해석한다."""
    parser = argparse.ArgumentParser(description="전국 신호등 데이터 로더")
