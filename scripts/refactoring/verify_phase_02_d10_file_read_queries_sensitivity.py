#!/usr/bin/env python
"""Negative-control evidence for the Phase 2-D10 FileReadQueries boundary."""
...
DEFAULT_EVIDENCE_PATH: Final = REPO_ROOT / "tmp" / "phase-02-d10-sensitivity-evidence.json"
TEST_MODULE: Final = "chatbot.test_phase_02_file_read_queries_use_case"
TARGETS: Final = {
    "list_view_application_bypass": TEST_MODULE
    + ".FileReadQueriesApplicationSeamTests."
    ...
    "detail_owner_authorization_bypass": TEST_MODULE
    + ".FileReadSecurityBoundaryTests."
...
application = repo_root / "app" / "application" / "files" / "read_queries.py"
if mutation_name == "list_view_application_bypass":
    mutation = mutate_once(
        views,
        "            result = execute_list_file_attachments(\n",
        "            from app.application.files.read_queries import execute_list_file_attachments as _direct_list\n            result = _direct_list(\n",
    )
elif mutation_name == "detail_view_application_bypass":
    mutation = mutate_once(
        views,
        "            result = execute_get_file_attachment(\n",
        "            from app.application.files.read_queries import execute_get_file_attachment as _direct_detail\n            result = _direct_detail(\n",
    )
...
elif mutation_name == "privacy_projection_bypass":
    mutation = mutate_once(
        application,
...
def build_evidence(
    *,
    ...
    return {
        "contract_version": "phase_02_d10_sensitivity.v1",
...
def main() -> int:
    ...
    requested_head = os.environ.get("PHASE_02_D10_SENSITIVITY_HEAD", "").strip()
    head = requested_head or actual_head
    try:
        if requested_head and requested_head != actual_head:
            raise SensitivityError("stale D10 sensitivity head: requested evidence head does not match checkout")
        original_exit_code = _run([sys.executable, "backend/manage.py", "test", TEST_MODULE, "--verbosity", "1"]).returncode
        if original_exit_code:
            raise SensitivityError(f"original D10 characterization suite failed: {original_exit_code}")
    ...
    path = Path(os.environ.get("PHASE_02_D10_SENSITIVITY_EVIDENCE_PATH", DEFAULT_EVIDENCE_PATH))
